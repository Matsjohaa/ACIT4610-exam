#!/usr/bin/env python3
"""
CLI entrypoint for RL (#5): Q-learning on FrozenLake-v1.
Covers: training, evaluation, plotting, baseline comparisons.  [Reqs: train long, compare maps,
baselines random/heuristic, plots, success rate metric].  See assignment. :contentReference[oaicite:3]{index=3}
"""
import argparse
...
def main():
    ...
    p_train.add_argument("--epsilon_schedule", choices=["linear","exp"], default="exp")
    p_train.add_argument("--alpha", type=float, default=0.8)
    ...
    cfg = load_config(args)  # merge CLI + defaults
    set_global_seeds(cfg.seed)

    if args.cmd == "train":
        run_dir = train_loop(cfg)  # returns Path
